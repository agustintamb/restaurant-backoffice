...
import { IDish, IUpdateDish } from '@/interfaces/dish';
...
interface DishEditModalProps {
  ...
  onClose: () => void;
  dish: IDish | null;
  onUpdate: (dishId: string, dishData: IUpdateDish) => void;
  isLoading?: boolean;
}

interface DishEditFormValues {
  name: string;
  description: string;
  price: number;
  image: File | null;
...
const DishEditModal = ({
  ...
  const [initialAllergenIds, setInitialAllergenIds] = useState<string[]>([]);

  // Set initial values when dish changes
  useEffect(() => {
    if (dish && open) {
      const categoryId = typeof dish.category === 'string' ? dish.category : dish.category._id;
      ...
      const ingredients = dish.ingredients.map(ing =>
        typeof ing === 'string' ? ({ _id: ing, name: ing } as IIngredient) : ing
      ) as IIngredient[];
      setInitialIngredients(ingredients);

      // Store initial ingredient IDs for comparison
      const ingredientIds = ingredients.map(ing => ing._id);
      setInitialIngredientIds(ingredientIds);

      // Set initial allergens
      const allergens = dish.allergens.map(allergen =>
        typeof allergen === 'string' ? ({ _id: allergen, name: allergen } as IAllergen) : allergen
      ) as IAllergen[];
      setInitialAllergens(allergens);

      // Store initial allergen IDs for comparison
      const allergenIds = allergens.map(allergen => allergen._id);
      setInitialAllergenIds(allergenIds);
    }
  }, [dish, open, setInitialIngredients, setInitialAllergens]);

  const initialValues: DishEditFormValues = {
    name: dish?.name || '',
    description: dish?.description || '',
    price: dish?.price || 0,
    image: null,
    categoryId: dish ? (typeof dish.category === 'string' ? dish.category : dish.category._id) : '',
    subcategoryId:
      dish && dish.subcategory
  ...
  // Check if there are any changes (form values, ingredients, allergens, or image)
  const hasChanges = useMemo(() => {
    if (!dish) return false;

    // Check ingredient changes
    const currentIngredientIds = getSelectedIngredientIds();
    const ingredientsChanged =
      currentIngredientIds.length !== initialIngredientIds.length ||
      currentIngredientIds.some(id => !initialIngredientIds.includes(id)) ||
      initialIngredientIds.some(id => !currentIngredientIds.includes(id));

    // Check allergen changes
    const currentAllergenIds = getSelectedAllergenIds();
    const allergensChanged =
      currentAllergenIds.length !== initialAllergenIds.length ||
      currentAllergenIds.some(id => !initialAllergenIds.includes(id)) ||
      initialAllergenIds.some(id => !currentAllergenIds.includes(id));

    // Check image changes
    const imageChanged = !keepCurrentImage;

    return ingredientsChanged || allergensChanged || imageChanged;
  }, [
    dish,
    getSelectedIngredientIds,
    getSelectedAllergenIds,
  ...
  const handleSubmit = async (values: DishEditFormValues) => {
    if (!dish) return;

    const updateData: IUpdateDish & { image?: File } = {
    ...
    if (!keepCurrentImage && values.image) {
      // New image selected
      updateData.image = values.image;
    } else if (!keepCurrentImage && !values.image) {
      // Remove image (send empty string)
      updateData.image = undefined;
    }
    // If keepCurrentImage is true, don't include image in update data

    onUpdate(dish._id, updateData);
  };
  ...
  const handleCategoryChange = (
    ...
    const subcategoryBelongsToCategory = filteredSubcategories.some(
      sub => sub._id === currentSubcategoryId
    );
  ...
  return (
    <Modal open={open} onClose={onClose} title="Editar Plato" size="xl">
      <Formik
      ...
      >
        {(formik: FormikProps<DishEditFormValues>) => (
          <Form className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {dish.image && !keepCurrentImage && !imagePreview && (
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-gray-200">
              ...
              <Button
                ...
                variant="primary"
                isLoading={isLoading}
                disabled={formik.isSubmitting || (!formik.dirty && !hasChanges)}
              >
                Guardar Cambios
              </Button>
            </div>
